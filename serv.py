...
def initialize_database():
    ...
    cursor.execute("CREATE DATABASE IF NOT EXISTS simple_shop_db;")
    cursor.execute("USE simple_shop_db;")
...
async def get_all_products():
    ...
    cursor.close()
    connection.close()
    return [Product(id=p['id'], name=p['name'], price=p['price'], quantity=p['quantity']) for p in products]
...
async def update_product(product_id: int, product: NewProduct):
    ...
    connection.close()
    if updated_product:
        return Product(id=updated_product['id'], name=updated_product['name'],
                       price=updated_product['price'], quantity=updated_product['quantity'])
    raise HTTPException(status_code=404, detail="Product not found")
